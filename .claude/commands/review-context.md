...
1. Find project root (works from subdirectories)
2. Source common-functions.sh
...
if [ "$QUICK_MODE" = "true" ]; then
  # Find project root (v5.2.0+)
  source scripts/common-functions.sh 2>/dev/null || {
    find_project_root() {
      local search_dir="$PWD"
      for i in 1 2 3 4 5; do
        [ -f "$search_dir/context/.context-config.json" ] && { cd "$search_dir" && pwd; return 0; }
  ...
  PROJECT_ROOT=$(find_project_root 2>/dev/null) && cd "$PROJECT_ROOT"
...
**Why this matters:** Allows command to work from subdirectories (up to 5 levels deep) by finding and cd'ing to project root. Provides access to performance-optimized functions, version checking, progress indicators, and standardized logging.
...
if [ -f "$CONTEXT_DIR/SESSIONS.md" ]; then
  ...
  # Note: grep -c outputs "0" if no matches, empty string on error; tr sanitizes whitespace
  SESSION_COUNT=$(grep -cE "^## Session [0-9]+" "$CONTEXT_DIR/SESSIONS.md" 2>/dev/null | tr -d ' \n')
  # Default to 0 if empty (grep error) or non-numeric
  ...
  if type get_max_session_number &>/dev/null; then
    MAX_SESSION=$(get_max_session_number "$CONTEXT_DIR" 2>/dev/null)
  else
    # Fallback if function not available: assume no gaps
    MAX_SESSION="$SESSION_COUNT"
  fi
  # Validate MAX_SESSION is numeric, default to SESSION_COUNT if not
  [[ "$MAX_SESSION" =~ ^[0-9]+$ ]] || MAX_SESSION="$SESSION_COUNT"

...
if [ ! -f "$CONTEXT_DIR/STATUS.md" ]; then
  ...
  echo "Quick Reference is auto-generated by /save from these sources."
  echo "If values are incorrect, update the source data, not Quick Reference directly."
  echo ""
  echo "Sources:"
  echo "  • Project name, URLs, tech stack → .context-config.json"
  echo "  • Current phase → STATUS.md ## Current Phase section"
  echo "  • Current focus → STATUS.md ## Active Tasks section"
  echo "  • Last session → SESSIONS.md most recent entry"
  echo ""
  echo "Verify Quick Reference accurately reflects current state."
  echo "If incorrect, update the source and run /save to regenerate."
  echo ""
fi
```
